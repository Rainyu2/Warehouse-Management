...
const rules = reactive<FormRules<typeof ruleForm>>({
  ...
  no: [{ required: true, message: '请输入账号', trigger: 'blur' },
    {required: true, message: '账号长度为2-8位', trigger: 'blur',min:2,max:8},
    {validator:checkDuplicate,trigger: 'blur'}],
  password: [{ required: true, message: '请输入密码', trigger: 'blur' },
    {required: true, message: '密码长度为3-8位', trigger: 'blur',min:3,max:8}],
  age:[{required: true, message: '请输入年龄', trigger: 'blur'},
    {required: true, message: '长度在1-3个位', trigger: 'blur',min:1,max:3},
...
const loadGetData = () => {
  if (instance) {
    ...
    const axiosInstance = instance.appContext.config.globalProperties.$axios;
    //双重请求只要有用的data数据
    axiosInstance.get('/list').then((res: AxiosResponse) => res.data)
        .then((res) => {
      console.log(res);
      if(res.code === 200){
  ...
  if (instance) {
    ...
    axiosInstance.post('/pageC1', {
      ...
      params: {
        ...
        .then((res) => {  // 这里应该已经是响应体数据，去掉AxiosResponse类型
          console.log(res);
          if(res.code === 200){
            tableData.value = res.data;
    ...
    >
        ...
        >
          <el-form-item label="账号" prop="no">
            <el-col :span="11">
              <el-input v-model="form.no" :disabled="!!form.no"/>
            </el-col>
          </el-form-item>
