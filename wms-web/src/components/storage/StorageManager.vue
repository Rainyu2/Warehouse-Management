...
const loadPostData = () => {
  if (instance) {
    ...
    axiosInstance.post('/storage/page', {
...
const del = (id:any)=>{
  ...
  if(instance){
    const axiosInstance = instance.appContext.config.globalProperties.$axios;
    axiosInstance.get('/storage/del?id='+id,form.value)
...
const doSave = () => {
  if(instance){
    const axiosInstance = instance.appContext.config.globalProperties.$axios;
    axiosInstance.post('/storage/save', form.value)
        .then((res: AxiosResponse) => res.data)
        .then(() => {
...
const doMod = () => {
  if(instance){
    const axiosInstance = instance.appContext.config.globalProperties.$axios;
    axiosInstance.post('/storage/update', form.value)
        .then((res: AxiosResponse) => res.data)
        .then(() => {
          centerDialogVisible.value = false;
          resetForm();
